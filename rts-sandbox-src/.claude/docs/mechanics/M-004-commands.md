...
## Открытые места
- Приказ, под который у юнита нет поведения, `ActionEnded` не пришлёт, и
  очередь встанет насовсем. Для приказа каста закрыто в T-004: его `Check()`
  спрашивает `UnitBehaviourManager.CanHandle` и выбрасывает команду заранее.
  Для остальных приказов дыра остаётся.
- Общего правила подписок по проекту нет — T-028 в 0.2.0. Подписки самого
  `UnitCommandManager` симметричны с T-004 (17 подписок, 17 отписок).
- Отписки в `OnDestroy` защищены от невыполненного `Start` во всём проекте
  (T-043), но общего правила подписок так и нет — T-028 в 0.2.0.
